...
public class myClassifier1 extends Classifier implements java.io.Serializable {
	...
	public myClassifier1(Instances m_instances, Classifier[] bestClassifiers,
	...
	public void buildClassifier(Instances data) throws Exception {
		...
		for (int i = 0; i < iterationNum; i++) {
			...
			for (int j = 0; j < more_instances.numInstances(); j++) {
				instanceResult = iterationClassifiers[i].classifyInstance(more_instances.instance(j));
			...
			for (int j = 0; j < less_instances.numInstances(); j++) {
				instanceResult = iterationClassifiers[i].classifyInstance(less_instances.instance(j));
	...
	protected Instances selectWeightQuantile() {
		...
		while(iterator.hasNext()&&number < lessLabelNum*2){
